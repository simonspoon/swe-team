...
The platform-detection logic, the tool-availability checks, and the per-platform
routing tables. Loaded by the `verification` SKILL.md "Detect" and "Route" workflow
steps.
...
Common project layouts nest platform files (e.g., `app/src-tauri/tauri.conf.json` for
Tauri, `app/vite.config.ts` for web frontends).
...
When multiple platforms are detected, verify ALL of them. A single-platform project
loads exactly one verify skill; a multi-platform project (Tauri = web + desktop) loads
the matched set.
...
Do NOT proceed with a platform whose tool is missing. If other platforms were also
detected and their tools ARE available, proceed with those and note the gap.
...
3. Screenshot, inspect, interact, verify per the ios-verify skill
...
3. Screenshot, inspect tree, interact, verify per the desktop-verify skill
...
3. `khora launch`, navigate, screenshot, inspect, verify per the web-verify skill
...
Only include sections for platforms that were detected. Do not include sections for
platforms that were not relevant to the project.
