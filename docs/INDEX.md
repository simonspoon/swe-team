# Project Documentation Index

This `INDEX.md` catalogs the visual documentation for the swe-team agent
suite. Charts are HTML — open
[index.html](index.html) in a browser for the navigable landing page, or open
individual files directly.

| Topic | File | When to read |
|-------|------|--------------|
| Agents & skills overview | [overview.html](overview.html) | Understanding which agents exist and which skills each one invokes |
| Task lifecycle | [workflows/task-lifecycle.html](workflows/task-lifecycle.html) | Tracing a task through the captured→refined→planned→done lifecycle |
| Orchestrate drain loop | [workflows/orchestrate.html](workflows/orchestrate.html) | Understanding how the orchestrator drains ready tasks from the queue |
| Verification pipeline | [workflows/verification.html](workflows/verification.html) | Following the verification-orchestrator skill workflow when checking task output |
| Release flow | [workflows/release.html](workflows/release.html) | Cutting a release — version bump, tag, and distribution steps |
